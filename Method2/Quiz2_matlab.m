...
% Plot histogram
hist_priors = histogram([zeros(1,rows_bg), ones(1,rows_fg)], ...
    'Normalization', 'probability');

% Compute MLE for pY_cheetah and pY_grass (pi_j = c_j/n)
...
% 8 best features: 
% - features where the sd between the 2 classes are similar
% - X1 because the means of the 2 classes are far apart 
%features = [33,34,35,36,37,38,39,40]; % WORST
%features =  [1,2,3,4,59,62,63,64];  % BEST
features = [1,11,14,23,25,27,32,40]; % ACTUALLY BEST
...
zigzag = readmatrix('../HW1/Zig-Zag Pattern.txt');
...
y_truth = imread('../HW1/cheetah_mask.bmp');
